...
int		check_exit(t_wolf *wolf)
{
	...
	{	
		if (wolf->map->map[(int)wolf->map->gen.y][(int)wolf->map->gen.x] == '1')
			mlx_string_put(wolf->mlx, wolf->win, WIN_WID / 2, WIN_LEN / 2, 0xFFFFFF, "This door has no power");
		else
		{
...
{
	if (sqrt(pow(wolf->player->pos.x - wolf->map->gen.x - 0.5, 2) + pow(wolf->player->pos.y - wolf->map->gen.y - 0.5, 2)) < 1.5)
	{
		if (wolf->map->map[(int)wolf->map->gen.y][(int)wolf->map->gen.x] == '1')
			mlx_string_put(wolf->mlx, wolf->win, WIN_WID / 2, WIN_LEN / 2, 0xFFFFFF, "Press (X) to turn on");
		else
			mlx_string_put(wolf->mlx, wolf->win, WIN_WID / 2, WIN_LEN / 2, 0xFFFFFF, "Press (X) to turn off");
		return (1);
	}
	return (0);
}
